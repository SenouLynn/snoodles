...
After identifying the target tool during intent extraction, read the matching section from [references/tool-routing.md](references/tool-routing.md) and apply its routing rules. Do not load the full file — find only the section for the identified tool.
