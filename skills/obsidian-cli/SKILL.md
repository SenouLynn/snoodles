...
The vault path and directory layout are defined in `config.json` at the snoodles plugin root. The session-start hook injects the vault path into context — you do not need to read config.json manually unless the vault path was not injected.

If `config.local.json` exists (gitignored), it overrides values from `config.json`. This lets each machine customize paths without touching the tracked config.
...
>
> Read `references/task-format.md` for task insertion rules, formats, required fields, and
> the quick-add flow for project tasks and events.
>
> Read `references/vault-conventions.md` for directory layout, project file structure,
> project name mappings, and when to use MCP vs CLI.
